...
cv::Mat If_ZebraCrossing(cv::Mat frame, cv::Mat draw)
{
        ...
        if (cv::contourArea(contours[i]) > min_area)
        {
            ...
            if (points.size() == 4)
            {
                ...
                if (len_to_wid_ratio < 6)
                {
                    ...
                    cv::putText(draw, "ZebraBlock", points[0], 1, 3, cv::Scalar(255, 255, 0), 2, 8);
                }
            }
        }
    }
    ...
    if (all_points.size() >= 16)
    {
        ...
        // 求置信度(凸包面积 ?= 斑马线块总面积*2)
        double confidence_level = (hull_area <= white_area * 2) ? (hull_area / white_area * 2) : (white_area * 2 / hull_area);
        // 文字信息
        ...
        */
        ...
        {
            // 视觉显示
            cv::putText(draw, text, rect[0], 1, 2, cv::Scalar(0, 255, 0), 2, 8);
            for (int j = 0; j < 4; j++)
            {
        ...
        {
            cv::putText(draw, text, rect[0], 1, 2, cv::Scalar(0, 0, 255), 2, 8);
            for (int j = 0; j < 4; j++)
            {
